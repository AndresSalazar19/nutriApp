...
def list_appointments(user_id: uuid.UUID = Query(...), role: Literal["patient", "nutritionist"] = Query(...), db: Session = Depends(get_db)):
    try:
...
def update_appointment(appointment_id: uuid.UUID, db: Session = Depends(get_db), data: AppointmentUpdateRequest = None):
    return AppointmentService.update(db, appointment_id, data)

@router.delete("/{appointment_id}", response_model=None)
def cancel_appointment(appointment_id: uuid.UUID, db: Session = Depends(get_db), user_id: uuid.UUID = None, reason: str = None):
    try:
