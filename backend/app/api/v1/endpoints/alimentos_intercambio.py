...
from app.schemas.alimento_intercambio import AlimentoIntercambioResponse, AlimentoIntercambioUpdate
...
def get_all(db: Session = Depends(get_db), _=Depends(get_current_user)):
    ...
    data = [
        AlimentoIntercambioResponse.model_validate(a).model_dump(mode="json") for a in alimentos
    ]
...
def update(
    alimento_id: int,
    body: AlimentoIntercambioUpdate,
    ...
    if not alimento:
        return JSONResponse(
            status_code=404,
            content=error_response(["Alimento no encontrado"], status_code=404).model_dump(),
        )
