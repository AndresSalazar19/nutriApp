...
function NutritionistSidebar({ activeNav, onNavChange }: { activeNav: string; onNavChange: (l: string) => void }) {
  const icons: Record<string, React.ReactNode> = {
    ...
    lock:      <svg className="w-4 h-4 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" /></svg>,
    settings:  <svg className="w-5 h-5 text-gray-400 hover:text-white transition" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.533 1.533 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clipRule="evenodd" /></svg>,
  };
    ...
    <aside className="w-56 bg-green-900 flex flex-col justify-between py-5 px-3 flex-shrink-0">
        ...
        <nav className="space-y-1">
          {navItems.map(({ label, icon, locked }) => {
            ...
            return (
              <button
                key={label}
                onClick={() => !locked && onNavChange(label)}
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-sm transition
                  ${isActive ? 'bg-green-500 text-white font-semibold' : 'text-green-200 hover:bg-green-800'}
      ...
      <div className="flex items-center justify-between px-2 py-2 rounded-lg hover:bg-green-800 transition cursor-pointer">
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 bg-green-500 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">AS</div>
          <div>
...
function MainView() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans">
      <NutritionistSidebar activeNav={activeNav} onNavChange={setActiveNav} />
        ...
        <div className="flex justify-center mb-6">
          <div className="relative w-36 h-36">
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 144 144">
              <circle cx="72" cy="72" r="68" fill="none" stroke="#f59e0b" strokeWidth="2" strokeDasharray="6 6" opacity="0.5" />
            </svg>
        ...
        <div className="grid grid-cols-3 gap-5 max-w-5xl mx-auto mb-8">
          <div className="flex items-start gap-4 bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div>
              ...
              <p className="text-left text-gray-400 text-xs">La verificación usualmente toma entre 24 y 48 horas hábiles</p>
            </div>
          </div>
          <div className="flex items-start gap-4 bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="flex items-start gap-4 bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div>
              ...
              <p className="text-left text-gray-400 text-xs">Recibirás un correo electrónico cuando tu cuenta sea aprobada</p>
            </div>
          </div>
        </div>
