...
import { ContentService, ContentItem, CONTENT_TYPES, CONTENT_CATEGORIES } from '../../services/Content/ContentService';
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {items.map(item => {
              ...
              const catLabel = CONTENT_CATEGORIES.find(c => c.value === item.category)?.label ?? item.category;
              const typeL    = CONTENT_TYPES.find(t => t.value === item.content_type)?.label ?? item.content_type;
              ...
              return (
                <div key={item.id}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {/* Badge de estado */}
                    <div className={`shrink-0 flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${config.badge}`}>
