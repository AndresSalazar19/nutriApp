...
function Metric({ label, value, sub, highlight }: {
  ...
  return (
    <div className="bg-gray-50 rounded-xl p-4 text-center">
      ...
      <p className={`text-2xl font-bold ${highlight ? bmiColor(parseFloat(value)) : 'text-gray-800'}`}>
        {value}
      </p>
    ...
    <div className="grid grid-cols-2 gap-6">
      ...
      <div>
        ...
        <div className="space-y-3 text-sm">
          <div>
            <p className="font-semibold text-gray-700 text-xs uppercase tracking-wide mb-1">Diagnóstico Principal</p>
            <p className="text-gray-600">{patient.diagnosis}</p>
          </div>
          {patient.additionalConditions.length > 0 && (
            <div>
              <p className="font-semibold text-gray-700 text-xs uppercase tracking-wide mb-1">Condiciones adicionales</p>
          ...
          <div>
            <p className="font-semibold text-gray-700 text-xs uppercase tracking-wide mb-1">Alergias e intolerancias</p>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-1">
            {['1M', '3M', '6M'].map(r => (
              <button key={r} className={`px-2 py-0.5 rounded text-xs font-medium transition
      ...
      <div className="col-span-2">
        ...
        <div className="grid grid-cols-6 gap-3">
          ...
          <Metric label="IMC" value={`${patient.bmi}`}
            sub={bmiLabel(patient.bmi)} highlight />
      ...
      <div className="col-span-2">
        <div className="flex items-center justify-between mb-3">
          ...
          <a href="#" className="text-green-600 text-xs hover:underline">Ver detalles →</a>
        </div>
        <div className="bg-gray-50 rounded-xl p-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <p className="text-lg font-bold text-green-700">{patient.nutritionalPlan.calories.toLocaleString()} kcal</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-gray-400 mb-0.5">Sodio/día</p>
              <p className="text-lg font-bold text-green-700">&lt; {patient.nutritionalPlan.sodium.toLocaleString()} mg</p>
            </div>
            <div className="text-center">
              <p className="text-xs text-gray-400 mb-0.5">Cumplimiento</p>
              <p className="text-lg font-bold text-green-700">{patient.nutritionalPlan.compliance}%</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-8 py-4 border-b border-gray-100 bg-white sticky top-0 z-10">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" icon={<span>💬</span>}>Mensaje</Button>
          <Button variant="primary" icon={<span>📅</span>}>Agendar</Button>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5">
          <div className="flex items-start gap-5">
            ...
            {/* Avatar */}
            <div className={`w-20 h-20 ${patient.color} rounded-full flex items-center justify-center
                ...
                <div className="flex gap-6 flex-shrink-0">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-green-500 mt-0.5">
                      {patient.adherence >= 80 ? '↑ Excelente' : patient.adherence >= 60 ? '→ Regular' : '↓ Bajo'}
                    </p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-gray-400 mb-1">Última consulta</p>
                    <p className="text-sm font-bold text-gray-800">{patient.lastConsult.split(',')[0]}</p>
                    <p className="text-xs text-gray-400">{patient.lastConsult.split(',')[1]?.trim()}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-gray-400 mb-1">Próxima cita</p>
                    <p className="text-sm font-bold text-gray-800">{patient.nextAppointment.split(',')[0]}</p>
                    <p className="text-xs text-gray-400">{patient.nextAppointment.split(',')[1]?.trim()}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between mt-5 border-t border-gray-100 pt-4">
            ...
            <Button variant="primary" icon={<span>🔔</span>}>Enviar Recordatorio</Button>
          </div>
        </div>
