...
import { TimePoint } from '../mock/reportsMock';
...
export function LineChart({ series, height = 160, showGrid = true, goalLine }: LineChartProps) {
  ...
  // Grid Y ticks
  const yTicks = Array.from({ length: 4 }, (_, i) => minVal + (i / 3) * (maxVal - minVal));

  function buildPath(data: TimePoint[]): string {
    return data
      .map(
        (d, i) =>
          `${i === 0 ? 'M' : 'L'} ${xScale(i, data.length).toFixed(1)} ${yScale(d.value).toFixed(1)}`,
      )
      .join(' ');
  }
    ...
    >
      ...
      {goalLine && (
        <>
          <line
            x1={PAD.left}
            y1={yScale(goalLine.value)}
            x2={W - PAD.right}
            y2={yScale(goalLine.value)}
      ...
      {xLabels
        .filter(
          (_, i) => i === 0 || i === xLabels.length - 1 || i === Math.floor(xLabels.length / 2),
        )
        .map((d, _, arr) => {
