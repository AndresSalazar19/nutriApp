...
import { WeightEntry } from './patientsMock';
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: H }}>
      ...
      {data.map((d, i) => (
        <circle
          key={i}
          cx={xScale(i)} cy={yScale(d.value)} r="3.5"
      ...
      <text
        ...
        textAnchor="middle" fontSize="10" fill="#16a34a" fontWeight="700"
      >
      ...
      {[minVal + 1, goal, maxVal - 1].map(v => (
        <text key={v} x={PAD.left - 4} y={yScale(v) + 4} textAnchor="end" fontSize="9" fill="#9ca3af">
          {v.toFixed(0)}
        </text>
      ))}
    </svg>
