...
export function ToCalendarAppointment(dto: AppointmentResponse): CalendarAppointment {
    ...
    return {
        ...
        // keep original dayIndex for backward compatibility
        dayIndex: (startDate.getDay() + 6) % 7, // lunes = 0

        // also store the exact start date/time as ISO so the UI can filter by week
